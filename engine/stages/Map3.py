...
class Map3(AbstractStage):
    def __init__(self, controller, window, sound, player, sprite, font, objects):
        ...
        super().__init__(player, window, "Map3", level_dimensions)
        self.destination = (self.window.windowScreen.get_height() * 0.32)
        self.bg_alpha_value = 0
        self.animation_value = 1
        self.shiplocation = (self.window.windowScreen.get_height() * 0.18)
        self.frameCount = 0
        self.time = 0
        self.font = None
        self.font_size = 30
    ...
    def screenCinema(self):
        ...
        if self.time > 5:
            ImageControl.setImageAt(self.window, self.image_nave, ((self.window.windowScreen.get_width() * 0.48), self.shiplocation))
    ...
    def update(self):
        while self.nextStageKey is "Map3":
