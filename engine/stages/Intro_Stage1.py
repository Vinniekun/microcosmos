...
class Intro_Stage1(AbstractStage):
    def __init__(self, controller, window, sound, player, sprite, font, objects):
        self.control = controller
        self.name = "Intro_Stage1"
        self.sound = sound
        self.control = controller
        level_dimensions = (1280,720)
        super().__init__(player, window, "Intro_Stage1", level_dimensions)
        ...
        self.file_lines = [18, 19, 20, 21, 22, 23, 24]
    ...
    def screenCinema(self):
        if self.time >= 1:
            ImageControl.setImageAt(self.window, self.text_render[0],
                                    ((self.window.windowScreen.get_width() // 2) - 100, 50))
        if self.time >= 3:
            ImageControl.setImageAt(self.window, self.text_render[1], (50, 150))
        if self.time >= 6:
    ...
    def update(self):
        while self.nextStageKey is "Intro_Stage1":
            fps = super().update_intros()
            self.eventTimer(fps)
            self.screenCinema()
            next = self.player.introControls()
            if next is True:
                self.nextStageKey = "Stage1"
