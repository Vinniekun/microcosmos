...
class Ameaca_3(AbstractStage):
    def __init__(self, controller, window, sound, player, sprite, font, objects):
        ...
        super().__init__(player, window, "Ameaca_3", level_dimensions)
        ...
        self.file_lines = [55, 56, 57, 58, 59, 60, 61, 62]
    ...
    def loadImages(self):
        self.image = pygame.image.load("../graphics/Monsters/sistemaimuno/sistemaimuno1.png").convert_alpha()
    ...
    def screenCinema(self):
        ...
        if self.time > 1:
            ImageControl.setImageAt(self.window, self.text_render[0], ((self.window.windowScreen.get_width() // 2) - 200, 50))
        if self.time > 2:
            ImageControl.setImageAt(self.window, self.image, ((self.window.windowScreen.get_width() * 0.05), 250))
            ImageControl.setImageAt(self.window, self.image, ((self.window.windowScreen.get_width() * 0.1), 200))
            ImageControl.setImageAt(self.window, self.image, ((self.window.windowScreen.get_width() * 0.15), 300))
        if self.time > 4:
            ImageControl.setImageAt(self.window, self.text_render[1], ((self.window.windowScreen.get_width() * 0.3), 200))
        if self.time > 6:
            ImageControl.setImageAt(self.window, self.text_render[2],
                                    ((self.window.windowScreen.get_width() * 0.3), 250))
        if self.time > 8:
            ImageControl.setImageAt(self.window, self.text_render[3],
                                    ((self.window.windowScreen.get_width() * 0.3), 300))
        if self.time > 10:
            ImageControl.setImageAt(self.window, self.text_render[4],
                                    ((self.window.windowScreen.get_width() * 0.3), 350))
        if self.time > 12:
            ImageControl.setImageAt(self.window, self.text_render[5],
                                    ((self.window.windowScreen.get_width() * 0.3), 400))
        if self.time > 14:
            ImageControl.setImageAt(self.window, self.text_render[6],
                                    ((self.window.windowScreen.get_width() * 0.3), 450))
        if self.time > 16:
            ImageControl.setImageAt(self.window, self.text_render[7],
                                    ((self.window.windowScreen.get_width() * 0.3), 500))
    ...
    def update(self):
        while self.nextStageKey is "Ameaca_3":
            fps = super().update_intros()
            self.eventTimer(fps)
            self.screenCinema()
            next = self.player.introControls()
            if next is True:
                self.nextStageKey = "Intro_Stage3"
