...
class Map(AbstractStage):
    def __init__(self, controller, window, sound, player, sprite, font, objects):
        ...
        super().__init__(player, window, "Map", level_dimensions)
        self.destination = self.window.windowScreen.get_width() * 0.4
        self.bg_alpha_value = 0
        self.animation_value = 1
        self.shiplocation = self.window.windowScreen.get_width() * 0.2
        self.frameCount = 0
        self.time = 0
        self.font = None
        self.font_size = 30
    ...
    def update(self):
        while self.nextStageKey is "Map":
