...
class Ameaca_2(AbstractStage):
    def __init__(self, controller, window, sound, player, sprite, font, objects):
        ...
        super().__init__(player, window, "Ameaca_2", level_dimensions)
        ...
        self.file_lines = [36, 37, 38, 39, 40, 41]
    ...
    def loadImages(self):
        self.image = pygame.image.load("../graphics/Monsters/pneumococo/pneumococo1.png").convert_alpha()
    ...
    def update(self):
        while self.nextStageKey is "Ameaca_2":
            fps = super().update_intros()
            self.eventTimer(fps)
            self.screenCinema()
            next = self.player.introControls()
            if next is True:
                self.nextStageKey = "Intro_Stage2"
