...
class Intro_Stage3(AbstractStage):
    def __init__(self, controller, window, sound, player, sprite, font, objects):
        self.control = controller
        self.name = "Intro_Stage3"
        self.sound = sound
        self.control = controller
        level_dimensions = (1280,720)
        super().__init__(player, window, "Intro_Stage3", level_dimensions)
        ...
        self.file_lines = [63, 64, 65, 66, 67, 68, 69, 70, 71]
    ...
    def screenCinema(self):
        if self.time >= 1:
            ImageControl.setImageAt(self.window, self.text_render[0],
                                    ((self.window.windowScreen.get_width() * 0.3), 50))
        if self.time >= 3:
            ImageControl.setImageAt(self.window, self.text_render[1], (50, 150))
        if self.time >= 6:
            ImageControl.setImageAt(self.window, self.text_render[2], (50, 200))
        if self.time >= 9:
            ImageControl.setImageAt(self.window, self.text_render[3], (50, 250))
        if self.time >= 12:
            ImageControl.setImageAt(self.window, self.text_render[4], (50, 300))
        if self.time >= 15:
            ImageControl.setImageAt(self.window, self.text_render[5], (50, 350))
        if self.time >= 18:
            ImageControl.setImageAt(self.window, self.text_render[6], (50, 400))
        if self.time >= 21:
            ImageControl.setImageAt(self.window, self.text_render[7], (50, 450))
        if self.time >= 24:
            ImageControl.setImageAt(self.window, self.text_render[8], (50, 500))
    ...
    def update(self):
        while self.nextStageKey is "Intro_Stage3":
