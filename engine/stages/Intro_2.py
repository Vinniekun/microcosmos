...
class Intro_2(AbstractStage):
    def __init__(self, controller, window, sound, player, sprite, font, objects):
        ...
        super().__init__(player, window, "Intro_2", level_dimensions)
    ...
    def loadImages(self):
        self.image = pygame.image.load("../graphics/Imagens/naveindo.png")
    ...
    def screenCinema(self):
        self.image.set_alpha(self.bg_alpha_value)
        if self.tremor:
            ImageControl.centerImage(self.window, self.image, self.offsetImage[0],self.offsetImage[0])
        else:
            ImageControl.centerImage(self.window, self.image)
        self.tremor = not self.tremor
    ...
    def update(self):
        ...
        while self.nextStageKey is "Intro_2":
